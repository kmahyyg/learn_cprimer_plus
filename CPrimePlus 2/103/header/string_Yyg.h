...
char b,
char c
) {         // From A to C with the help of B
if (n == 1)                                          // 1 Disc? A to C.
{
printf("%c-->%c\n", a, c);
} else {
hanoi(n
- 1, a, c, b);                  // migrate n-1 disc from A to B with the help of C
printf("%c-->%c\n", a, c);               // migrate last disc from A to C
hanoi(n
- 1, b, a, c);                  // migrate n-1 disc from B to C with the help of A
}
}
#endif //HANOI_T
